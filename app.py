...
def auto_step_env(api_key, base_url, model_name):
    ...
    try:
        with open("openenv.yaml", "r") as f:
            meta = yaml.safe_load(f)
        task_desc = next(
            (t["description"] for t in meta["tasks"] if t["id"] == current_task_id), ""
        )
...
with gr.Blocks(title="OpenEnv - HR Compliance") as demo:
    ...
    gr.Markdown(
        "Simulate actions as an HR compliance officer interacting with the employee report inbox system."
    )

    with gr.Accordion("Settings: Model & API", open=False):
        gr.Markdown(
            "Configure the AI inference runner for the Auto-Step functionality."
        )
        with gr.Row():
            ...
            model_input = gr.Textbox(label="Model Name", value=MODEL)
    ...
    with gr.Row():
        action_input = gr.Textbox(
            label="Action JSON Input",
            lines=3,
            placeholder='{"action_type": "read", "item_id": "101", "payload": null}',
        )
